...
- 9 relational tables, ~100K orders, Oct 2016 to Sep 2018
...
was excluded from all growth calculations since it contained only 1 order,
...
segmentation behaves as expected: Champions cluster at high recency and high
...
- **Target the "Lost" and "At Risk" segments (~32% of customers) with a
  win-back campaign** rather than treating all non-Champions equally, since
  these two segments were previously engaged, making them cheaper to
  re-activate than acquiring new customers.
...
- **Treat one-time-purchase behavior as the default, not the exception.**
  With second-month retention under 1% across two full cohort years, broad
  loyalty-program investments aimed at "increasing repeat purchases"
  generally are unlikely to pay off; a more targeted approach (e.g. focused
  on the New/Promising RFM segment specifically, where a customer has just
  shown purchase intent) is more likely to move the needle than a
  platform-wide push.

## SQL techniques used
CTEs, window functions (`LAG`, `RANK`, `NTILE`), multi-table joins,
date-truncation cohort logic, and views as a clean SQL-to-BI handoff layer.
...
## Limitations
- Dataset covers a fixed historical window (Oct 2016 to Sep 2018); "current"
